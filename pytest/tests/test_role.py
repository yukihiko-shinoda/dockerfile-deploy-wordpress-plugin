...
class TestRole:
    def test_rsync_git_to_svn_default_rsync_filter(
        self, git_repository_url, git_svn_checkout_workspace, clear_artifacts, playbook_tasks
    ):
        self.run_role()
    ...
    @pytest.mark.parametrize('src, dest', [
        ['.rsync-filter', '/root/workdir/git-working-dir/.rsync-filter'],
    ])
    def test_rsync_git_to_svn_project_rsync_filter(
        self, git_repository_url, git_svn_checkout_workspace, fixture_file, clear_artifacts, playbook_tasks
    ):
        self.run_role()
        assert os.path.exists('/root/workdir/my-local-dir/trunk/plugin.php')
        assert not os.path.exists('/root/workdir/my-local-dir/trunk/.git')
